...
-- Add columns to profiles table to store mistake pattern insights
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS mistake_pattern_insight TEXT,
ADD COLUMN IF NOT EXISTS mistake_pattern_date DATE,
ADD COLUMN IF NOT EXISTS mistake_pattern_type TEXT;

-- Create a function to analyze user's mistake patterns
...
-- Create a function to generate a mistake-pattern mission for a user
CREATE OR REPLACE FUNCTION public.generate_mistake_pattern_mission(target_user_id UUID, pattern_type TEXT, pattern_description TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  ...
  xp_reward INTEGER := 75;
  expire_time TIMESTAMP WITH TIME ZONE;
BEGIN
  -- Set expiration to end of next week
  expire_time := (CURRENT_DATE + INTERVAL '7 days' + INTERVAL '23 hours 59 minutes');
  
  -- Generate mission based on pattern type
  IF pattern_type = 'day_pattern' THEN
    mission_title := 'Break the Day Pattern';
    mission_description := format('Complete 3 profitable trades to overcome your %s pattern', pattern_description);
    target_value := 3;
  ELSIF pattern_type = 'revenge_trading' THEN
    mission_title := 'Emotional Control Challenge';
    mission_description := 'Wait at least 1 hour between trades to avoid revenge trading (complete 5 patient trades)';
    target_value := 5;
  ELSIF pattern_type = 'quick_trades' THEN
    mission_title := 'Patience Builder';
    mission_description := 'Hold trades for at least 1 hour before closing (complete 3 patient trades)';
    target_value := 3;
  ELSE
    mission_title := 'Discipline Master';
    mission_description := 'Focus on following your trading plan with 5 well-planned trades';
    target_value := 5;
  END IF;
